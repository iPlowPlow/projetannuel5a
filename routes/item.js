module.exports = function (app, models) {
  ...
  app.get("/item", function(req, res, next) {
    if (req.body.idItem){
      ...
      sequelize.query("SELECT price, location, city, quantity, item.name as itemName, description, photoURL, loginUser, category.name as categoryName, product.name as productName, unit.name as unitName, idProducer FROM item, product, category, unit, user, producer WHERE item.idUser = producer.idUserProducer AND item.idUser = user.idUser AND item.idProduit = product.id AND item.unitId = unit.id AND product.categorieId = category.id AND item.id = :idItem ",{ replacements: { idItem:  req.body.idItem }, type: sequelize.QueryTypes.SELECT  })
        .then(function(result){
            if(result){
              jsonResult.code = 0;
